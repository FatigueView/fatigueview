...
class MultiModel(nn.Module):
    def __init__(self):
        ...
        self.eye_alex = models.alexnet(pretrained=True)
        self.eye_alex.classifier = nn.Sequential()

        # num_ftrs = self.eye_alex.classifier[6].in_features
        # self.eye_alex.classifier[6] = nn.Linear(num_ftrs, 2)


        self.mouth_alex = models.alexnet(pretrained=True)
        self.mouth_alex.classifier = nn.Sequential()

        # num_ftrs = self.mouth_alex.classifier[6].in_features
        # self.mouth_alex.classifier[6] = nn.Linear(num_ftrs, 2)
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        try:
            eye = cv2.imread(self.data_list[index])
            mouth = cv2.imread(self.data_list[index].replace('eye.jpg','mouth.jpg'))
        ...
        if np.sum(mouth) < 10  and (np.sum(eye) < 10 or label == 2):
            return self.blank_img, self.blank_img, -1, index
        ...
        eye = eye[:,:,::-1]
        eye =  Image.fromarray(eye.astype('uint8'))
        eye = transform(eye)
        try:
            mouth = mouth[:,:,::-1]
            mouth =  Image.fromarray(mouth.astype('uint8'))
...
def train(model,data_list,label_list,epoch,model_prefix):
    ...
    scheduler = lr_scheduler.OneCycleLR(optimizer_ft, max_lr=0.01, steps_per_epoch=len(dataloader), epochs=epoch)
...
def inference(model,data_list,label_list):
    ...
    dataloader = torch.utils.data.DataLoader(data,batch_size=256,shuffle=False,num_workers=32,pin_memory=True)
    ...
    for imgs,flows,labels,index_list in dataloader:
        ...
        with torch.set_grad_enabled(False):
            ...
            for index,output,label in zip(index_list,outputs,labels):
