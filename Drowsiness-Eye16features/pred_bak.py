...
def eval_pr(iou,threshold):
    ...
    TP = 0
    while True:
        if iou.shape[0] * iou.shape[1] == 0:
            break
        if np.max(iou) < threshold:
            break

        pos = np.unravel_index(np.argmax(iou), iou.shape)
        TP += 1
        iou = np.delete(iou,pos[0],axis = 0)
        iou = np.delete(iou, pos[1], axis=1)
...
def line_IOU(pred,label):
    ...
    for i,pred_slice in enumerate(pred):
        for j,label_slice in enumerate(label):
            ...
            out_left = min(pred_slice[0], label_slice[0])
            out_right = max(pred_slice[1], label_slice[1])

            iou[i,j] = (temp_right - temp_left) / (out_right - out_left + 1e-5)
