...
def get_video_slice_dict(max_len,frame_len = 15000 ,setp_frame = 15000):
    video_slice_dict = {}
...
def get_batch_data(video_list,suffix,batch_size,start_id,dst_dir,stage= 'train',time_len = 1,random_ratio = 0.5):
    ...
    while True:
        ...
        if stage == 'train':
            ...
            if os.path.exists(poor_acting_path):
                with open(poor_acting_path, 'r') as f:
                    ...
                    temp_poor_action_list = temp_poor_action_list[
                        os.path.basename(video_path).replace(suffix, '')]
        ...
        for i in range(1,frame_len,8):
            ...
            if i not in video_slice_dict:
                continue

            video_id = video_slice_dict[i]
            ...
            flow_name = '{}__{}__{}_{}__{}.npy'.format(temp_label,label,video_id,base_name, str(i).zfill(8))
            np.save(os.path.join(dst_dir, flow_name), flow)


            frame_name = '{}__{}__{}_{}__{}.jpg'.format(temp_label,label,video_id,base_name, str(i).zfill(8))
