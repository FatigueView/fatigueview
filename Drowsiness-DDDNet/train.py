...
if __name__ == '__main__':
    ...
    weights = '/mnt/data-1/zhenyu.yang/FatigueViewBaseline/Park2016DDD/weights'
    if not os.path.exists(weights):
        os.makedirs(weights)

    camera_list = ['ir_down','ir_front','ir_left','ir_left_up','ir_up','rgb_down','rgb_front','rgb_left','rgb_left_up','rgb_up']
    camera_list = ['ir_down','ir_front','ir_left','ir_left_up','ir_up','rgb_down','rgb_up']
    camera_list = ['ir_down','ir_front','ir_left','ir_left_up','ir_up','rgb_down']
    ...
    for camera_id in range(len(camera_list)):
        ...
        if camera_list[camera_id] == 'rgb_up':
            src_dir_dict['train'] = '/mnt/data-1/zhenyu.yang/FatigueViewBaseline/Park2016DDD/data/train'

        train_data_dir = os.path.join(src_dir_dict['train'],camera_list[camera_id])
        train_data_list = getFiles(train_data_dir,'.jpg')
        label_list = [int(os.path.basename(v)[0]) for v in train_data_list]
        train_x,train_y = data_balance(train_data_list,label_list)
        model = MultiModel()
        model = train(model,train_x,train_y,15,camera_list[camera_id])
