...
def load_data(data_list):
    ...
    for data_path in data_list:
        with open(data_path,'r') as f:
            data = json.load(f)
        all_data.append([data['eye_label'],data['raft_eye_label'],data['label']])
...
if __name__ == '__main__':
    ...
    for camera_id in range(len(camera_list)):
        ...
        precison = sum(y_ == y and y_ == 0 for y_,y in zip(pred,test_y)) / (sum(y_ == 0 for y_ in pred) + 0.0001)
        recall = sum(y_ == y and y_ == 0 for y_,y in zip(pred,test_y)) / (sum(y == 0 for y in test_y) + 0.0001)
