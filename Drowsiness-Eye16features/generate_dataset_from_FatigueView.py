...
def get_batch_data(video_list,suffix,batch_size,start_id,dst_dir,stage= 'train',time_len = 30,random_ratio = 0.5):
    ...
    while True:
        ...
        array_len = max(max(awake_list+[-1]),max(fatigue_list+[-1]),len(height_list)) + 1
        ...
        for i in range(len(height_list)):
            ...
            if i < half_frame_len or i >= len(height_list) - (half_frame_len+1):
                continue


            awake_time = 0
            fatigue_time = 0
            # for i in range(i-half_frame_len,i+half_frame_len):
            #     if i in awake_list:
            #         awake_time += 1
            ...
            # print('0 - min time:{:.3f}'.format(time.time() - min_tic))
            # print('0 - min time:{:.3f}'.format(time.time() - min_tic))
            ...
            temp_heights = height_list[i - half_frame_len:i + half_frame_len]



            bin_list = list2bin(temp_heights,min_height,max_height)

            # print('1 - min time:{:.3f}'.format(time.time() - min_tic))

            average_blink_duration,_ = get_blink_info(blink_all,i - half_frame_len,i + half_frame_len)
            average_closed_duration,micro_sleep = get_blink_info(close_list,i - half_frame_len,i + half_frame_len)
            average_close_duration,_ = get_blink_info(blink_close,i - half_frame_len,i + half_frame_len)
            average_open_duration, _ = get_blink_info(blink_open,i - half_frame_len,i + half_frame_len)

            # print('2 - min time:{:.3f}'.format(time.time() - min_tic))
            ...
            temp_feas = bin_list + [average_blink_duration,average_closed_duration,micro_sleep,average_close_duration,average_open_duration,label]
            temp_feas.append(data_id)
            ...
            # print('3 - min time:{:.3f}'.format(time.time()-min_tic))

        # print('ALL time:{:.3f}'.format(time.time() - tic) )
        if len(video_list) == 0:
            break
