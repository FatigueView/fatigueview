...
def get_batch_data(video_list,suffix,batch_size,start_id,dst_dir,gpu_id=0,random_ratio = 0.05):
    ...
    while True:
        ...
        while True:
            ...
            flow_y = flow[:, :, 0]
            left_eye_label, right_eye_label = get_doube_eye_status(flow_y,
                                                                   [left_eye,
                                                                    right_eye,
                                                                    thres])
            ...
            raft_left_eye_label, raft_right_eye_label = get_doube_eye_status(raft_flow_y,
                                                                   [left_eye,
                                                                    right_eye,
                                                                    thres])
            ...
            temp_info = {'thres': thres,
                         ...
                         'left_eye_label':np.float(left_eye_label),
                         'right_eye_label':np.float(right_eye_label),
                         'raft_left_eye_label':np.float(raft_left_eye_label),
                         'raft_right_eye_label':np.float(raft_right_eye_label),
...
if __name__ == '__main__':
    ...
    for camera_id in range(len(camera_list)):
        ...
        for i in range(running_num):
            temp_p = Process(target=get_batch_data,args=(split_videos[i],suffix,batch_size,batch_size*i,dst_dir,i%4))
